...
public class RAMDirectoryDemo {
	...
	public static void main(String[] args) throws IOException {
		 ...
		 * maxFieldLength
		 * :这个是分词器拆分最大长度，因为各种不同类型的分词器拆分的字符颗粒细化程度不一样，所以需要设置一个最长的拆分长度
		 * 。IndexWriter.MaxFieldLength.UNLIMITED表示无限制；
		 */
		 ...
		 *  Field.Index.ANALYZED: 分词建索引  Field.Index.ANALYZED_NO_NORMS:
		 * 分词建索引，但是Field的值不像通常那样被保存，而是只取一个byte，这样节约存储空间
		 *  Field.Index.NOT_ANALYZED: 不分词且索引
		 * ,一旦指定为这种类型后将会被lucenn录入索引中，但不会被作为关键搜索，除非输入所有的关键字
		 * Field.Index.NOT_ANALYZED_NO_NORMS: 不分词建索引，Field的值去一个byte保存
		 */

		doc.add(new Field("name", "Cheng Hui", Field.Store.YES,
				Field.Index.ANALYZED));
		doc.add(new Field("sex", "男的", Field.Store.YES,
